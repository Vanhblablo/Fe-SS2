    ...
    <div className={`${data.categoryListDropdown ? "" : "hidden"} my-4`}>
      ...
      <div className="py-1 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        {categories && categories.length > 0 ? (
          ...
          <div className="text-2xl text-center my-4">No Category</div>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div
      className={`${data.searchDropdown ? "" : "hidden"
        } my-4 flex items-right justify-between`}
    >
      <input
        value={search}
        onChange={(e) => searchHandle(e)}
        className="px-4 text-xl py-4  focus:border-blue focus:ring-1 focus:ring-sky"
        type="text"
        placeholder="Search products..nnn."
      />
